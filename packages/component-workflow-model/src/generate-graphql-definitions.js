...
`;
    ...
    if(stateElements && stateElements.length) {
        mutation += tab + `completeTaskFor${taskDef.name}(id:ID!, taskId:ID!, form:String!, outcome:String!, state:${stateInputTypeName}) : CompleteTaskOutcome\n`;
        mutation += tab + `destroy${taskDef.name}(id:ID, state:${stateInputTypeName}) : Boolean\n`;
    } else {
        mutation += tab + `completeTaskFor${taskDef.name}(id:ID!, taskId:ID!, form:String!, outcome:String!) : CompleteTaskOutcome\n`;
